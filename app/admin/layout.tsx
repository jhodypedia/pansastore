...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { name: "Transaksi", path: "/admin/transactions", icon: "ri-exchange-dollar-fill" },
    { name: "Katalog", path: "/admin/products", icon: "ri-box-3-fill" },
    { name: "Pengguna", path: "/admin/users", icon: "ri-group-fill" },
  ];

  const settingMenu = [
    { name: "Pengaturan API", path: "/admin/settings", icon: "ri-settings-4-fill" },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans selection:bg-emerald-600 selection:text-white relative overflow-x-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-white border-r border-slate-200 z-50 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.01)] transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-72 translate-x-0" : "w-20 -translate-x-full lg:translate-x-0"}`}>
        ...
        <div className={`h-20 flex items-center border-b border-slate-100 transition-all duration-300 ${isSidebarOpen ? "px-6 justify-between" : "px-0 justify-center"}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <span className={`font-black text-base tracking-tight text-slate-900 whitespace-nowrap transition-all duration-300 ${isSidebarOpen ? "opacity-100 max-w-full" : "opacity-0 max-w-0 overflow-hidden"}`}>
              PANSA GROUP
            </span>
          </div>
          <button className="lg:hidden p-1 text-slate-400 hover:text-slate-900 rounded-lg" onClick={() => setIsSidebarOpen(false)}>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-3 flex flex-col gap-7 scrollbar-none">
          <div>
            ...
            <nav className="space-y-1">
              {menuItems.map((item) => {
                ...
                return (
                  <Link key={item.path} href={item.path} onClick={() => window.innerWidth < 1024 && setIsSidebarOpen(false)} className={`flex items-center gap-3 py-3 rounded-xl text-sm font-bold transition-all duration-200 group relative ${isActive ? "bg-[hsl(var(--primary))] text-white shadow-md shadow-emerald-950/10" : "text-slate-500 hover:bg-slate-50 hover:text-[hsl(var(--primary))]"} ${isSidebarOpen ? "px-4 justify-start" : "px-0 justify-center"}`}>
                    <i className={`${item.icon} text-xl transition-transform duration-200 group-hover:scale-110`}></i>
                    <span className={`transition-all duration-300 whitespace-nowrap ${isSidebarOpen ? "opacity-100 max-w-full translate-x-0" : "opacity-0 max-w-0 overflow-hidden -translate-x-4"}`}>{item.name}</span>
                  </Link>
                );
              })}
            </nav>
          ...
          <div>
            <div className={`text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 whitespace-nowrap transition-all duration-300 ${isSidebarOpen ? "px-3 opacity-100" : "opacity-0 h-0 overflow-hidden mb-0"}`}>Sistem</div>
            <nav className="space-y-1">
      ...
      <main className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ease-in-out w-full max-w-full ${isSidebarOpen ? "lg:pl-72" : "lg:pl-20"}`}>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button className="text-slate-500 hover:text-[hsl(var(--primary))] transition-all duration-200 p-2 rounded-xl hover:bg-slate-100 active:scale-95 cursor-pointer" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
              <i className={`ri-menu-2-line text-2xl block transition-transform duration-300 ${isSidebarOpen ? "rotate-180" : "rotate-0"}`}></i>
            </button>
            <h1 className="font-extrabold text-base text-slate-900 tracking-tight hidden sm:block">
              {menuItems.concat(settingMenu).find(m => m.path === pathname)?.name || "Dashboard Admin"}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3 lg:gap-4 relative" ref={headerActionsRef}>
            ...
            <div className="flex items-center gap-3 pl-3 lg:pl-4 border-l border-slate-200 relative">
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  setActiveDropdown(activeDropdown === 'profile' ? 'none' : 'profile');
                }}
                ...
                <div className="absolute right-0 top-[120%] mt-2 w-64 bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-50 animate-fade-up ring-1 ring-black/5">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="p-2">
                      ...
                      <button onClick={() => { setActiveDropdown('none'); handleLogout(); }} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-red-600 hover:bg-red-50 w-full text-left transition-all duration-150 cursor-pointer">
